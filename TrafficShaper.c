 ...
 * File			         :	TrafficShaper.c
 * Description 	:	The file implements TrafficShaper
 *
 */
...
/* Print error message corresponding to erro codes */
...
/* Check and get value for each command option from command line */
             ...
             CallErr(0,0);
             else
               {
                   ...
                   while(count)
                  {
                        if((int)s[i]==45 )
                        CallErr(0,0);
                        else if(!((((int)s[i]>=48 )&& ((int)s[i]<=57)) || ((int)s[i]==46)))
                        CallErr(2, (void*)argv);
                         i++;count--;
                  }
                   *var = atof(argv);
...
/* Check and get data for each line from testfile */
void CheckAndGetData(char* buf, int line_number, int* data)
{
      ...
      count = strlen(buf);
             ...
             {
                strncpy(s, &buf[i],1);
                if(!((((int)s[0]>=48 )&& ((int)s[0]<=57)) || ((int)s[0]==43)))
...
/* Parse each line of the test file */
MyParsedata* ParseTraceFile()
{
 ...
 /* Check the command mode and get the values for each command*/
 void CheckCommandAndGetMode(int argc, char** argv)
 {
         ...
         if(argc <1)
          CallErr(0,0);
         /* Command Usage : warmup2 [-lambda lambda] [-mu mu] [-r r] [-B B] [-P P] [-n num] [-t tsfile]  */
         else if(argc > 1)
         {
                while( argv[j] )
               {
                         while(s[k])
                        {
                            if( !strcmp((const char*)argv[j], (const char*)s[k++]))
                            {
                                ...
                                CheckAndGetValue(argv[++j], &Parameter[k-1]);
                         ...
                         if( mode ==  TRACE_DRIVEN)
                         {
                               ...
                               file = fopen(filename, "r");
                                ...
                                {
                                    if(errno == ENOENT)
                                    CallErr(3,(void *)filename);
                                    else if(errno == EACCES)
                                    CallErr(4,(void *)filename);
                                }
                                stat( (const char*) filename, &statbuf );
                                if(S_ISDIR(statbuf.st_mode))
                                CallErr(5,(void *)filename);
                        ...
                        if(!flag && argv[j+1] )
                        {
                                k=0;
                                 while(s[k])
                                 {
                                      if(!strcmp((const char*)argv[j+1], (const char*)s[k++]))
                                      CallErr(0,0);
                                 }
                        }
             ...
             /* Check Parameters and initialise default values for undefined Parameters */
             for(k=0;k<6;k++)
            {
                  if (k>=3 && k <= 5 && (Parameter[k] != (double)((int)Parameter[k])))
                  CallErr(8,(void*)s[k]);

                 if(!Parameter[k])
                 Parameter[k] = Default_Params[k];
            }
...
/* Implementation of Packet Arrival Thread */
 void* Packet_Arrival()
{
      ...
      while(count< num)
      {
            ...
            if  (mode == DETERMINISTIC)
           {
                 ...
                 if(x>10000000)
                 InterArrivalTime = 10000000;
                 else
                 InterArrivalTime = round(x);
          ...
          else
          {
               ...
               t2= (tv.tv_sec *1000000) + (unsigned long)tv.tv_usec - t1;
                   ...
                   if((int)(InterArrivalTime-t2)>0)
                    usleep(InterArrivalTime-t2);
                ...
                if  (mode == DETERMINISTIC)
                {
                    ...
                    if(x>10000000)
                    Packet_p->ServiceTime = 10000000;
                    else
                    Packet_p->ServiceTime = round(x);
                }
                else
                {
                    Packet_p->InterArrivalTime = InterArrivalTime;
             ...
             pthread_mutex_lock(&mutex);
              if (My402ListEmpty(&Q2) &&  My402ListEmpty(&Q1))
                   signal_waiting=1;
            pthread_mutex_unlock(&mutex);
...
/* Implementation of Token Arrival Thread */
 void* Token_Arrival()
{
       int i=1;
...
/* Implementation of Server Thread */
 void* Server()
{
    ...
    int i=0;
           ...
           while(1)
           {
                ...
                if(My402ListEmpty(&Q2) &&  My402ListEmpty(&Q1) && count_processed  >= num)
                {
                    pthread_mutex_unlock(&mutex);
                    break;
                }
                ...
                My402ListElem *Packet =  My402ListFirst(&Q2);
                MyPacketList *Packet_t = (MyPacketList*)Packet->obj;

                My402ListUnlink(&Q2,Packet);
               gettimeofday(&tv,0);
               time_in_mill = (tv.tv_sec) * 1000000 + tv.tv_usec;
               ...
               printf("%012.03fms: p%d departs from S, service time = %.3fms, time in system = %.3fms\n", get_time(time_in_mill - Reftime), Packet_t->Packet_num,get_time(difftime), get_time(time_in_mill - Packet_t->MeasArrivalTime) );
                ...
                average_of_the_square_time_in_system= ((average_of_the_square_time_in_system*i)+(get_time(time_in_mill - Packet_t->MeasArrivalTime)*get_time(time_in_mill - Packet_t->MeasArrivalTime)))/(i+1);
...
pthread_mutex_lock(&mutex);
     ...
     {
          My402ListElem *Packet =  My402ListFirst(&Q1);
          num =  ((MyPacketList*)Packet->obj)->Packet_num;
          My402ListUnlink(&Q1, Packet);
          gettimeofday(&tv,0);
          time_in_mill = tv.tv_sec * 1000000 + tv.tv_usec ;
          printf("%012.03fms: p%d removed from Q1\n", get_time(time_in_mill - Reftime), num);
     }
...
void StopEmulation()
{
        ...
        printf("    average packet service time = N/A   no packet at Server\n");
        else
        ...
        printf("\n    token drop probability = %.6g\n",(double)((double)tokens_dropped/(tokens_added+tokens_dropped)) );
         ...
         printf("    packet drop probability = N/A  no packet arrived at the System\n" );
         else
...
void StartEmulation()
{
   ...
   Bucket_Tokens = 0;
     ...
     printf("00000000.000ms: emulation begins\n");
      ...
      void *result_Packet_arrival, *result_Token_Arrival, *result_Server, *result_SigInt_interrupt;
   ...
   pthread_join(Packet_arrival_thread, (void**)&result_Packet_arrival );
   pthread_join(Token_Arrival_thread, (void**)&result_Token_Arrival );
   ...
   pthread_join(Server_thread, (void**)&result_Server );

   if(SigInt)
   pthread_join(SigInt_interrupt_thread, (void**)&result_SigInt_interrupt );
   else
   pthread_cancel(SigInt_interrupt_thread);
